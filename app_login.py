...
class AppPrincipal(Tk):
    ...
    def mostrar_tela_login(self):
        ...
        Label(self.container, text="Acesse o sistema para continuar", font=bold_font).pack(
            pady=20
        )
        Button(
            self.container,
    ...
    def _worker_login_token(self):
        ...
        try:
            ...
            try:
                login_classico = wait.until(
                    EC.element_to_be_clickable(
                        (By.XPATH, "//span[contains(text(), 'Login Clássico')]")
                    )
                )
            ...
            if code_match:
                ...
                token_info = obter_token(
                    code=auth_code, silent_mode=False, base="homologacao"
                )
                if token_info and "access_token" in token_info:
                    RESULTADO_QUEUE.put(
                        {"status": "sucesso_login_token", "token_info": token_info}
                    )
                else:
                    raise Exception(
                        f"Falha ao obter o token de acesso. Resposta: {token_info}"
                    )
            else:
    ...
    def _worker_buscar_processos(self):
        try:
            ...
            payload = {
                "idsLocaisAtuais": ["b7926835-d3ac-48c9-a1a5-320c85b98a24"],
    ...
    def verificar_resultado_api(self):
        try:
            ...
            if resultado["status"] == "sucesso_api":
                ...
                processos_originais = self.dados_processos.get("result", [])

                TERMOS_FILTRO = ("DEFESA PRÉVIA", "DEFESA PREVIA")
                processos_filtrados = [
                    p
                    for p in processos_originais
                    if not any(
                        termo in p.get("resumo", "").upper() for termo in TERMOS_FILTRO
                    )
                ]
                ...
                print("\n--- PROCESSOS FILTRADOS PRONTOS PARA AUTOMAÇÃO ---")
                print(
                    json.dumps(
                        self.dados_processos["result"], indent=2, ensure_ascii=False
                    )
                )
                print("--------------------------------------------------\n")
                ...
                if self.dados_processos["count"] > 0:
                    print(
                        f"✅ API OK. {self.dados_processos['count']} processos para automação."
                    )
                    self.iniciar_login_renach()
                else:
                    # --- ADICIONADO: Fecha o navegador se não houver processos ---
                    if self.main_driver:
                        print("✅ Nenhum processo válido. Fechando o navegador...")
                        try:
                    ...
                    messagebox.showinfo(
                        "Concluído", "Nenhum processo válido encontrado após o filtro."
                    )
                    self.mostrar_tela_login()
            else:
                messagebox.showerror(
                    "Falha na API", resultado.get("erro", "Erro desconhecido.")
                )
                self.mostrar_tela_login()
        except queue.Empty:
    ...
    def mostrar_tela_login_renach(self):
        ...
        Label(
            self.container,
            text="Realizando login no portal RENACH...\n\nAguarde um momento.",
            font=bold_font,
        ).pack(pady=50)
    ...
    def _worker_login_renach(self):
        try:
            ...
            wait.until(
                EC.element_to_be_clickable(
                    (By.XPATH, "//div[contains(text(), 'Entrar com o Acesso Cidadão')]")
                )
            ).click()
            try:
                wait.until(
                    EC.element_to_be_clickable(
                        (By.ID, "ctl00_conteudo_btnContinuarAcessoCidadao")
                    )
                ).click()
            except TimeoutException:
                pass
    ...
    def verificar_resultado_login_renach(self):
        try:
            ...
            if resultado["status"] == "sucesso_login_renach":
                ...
                messagebox.showerror(
                    "Falha no RENACH", resultado.get("erro", "Erro desconhecido.")
                )
                self.mostrar_tela_login()
        except queue.Empty:
    ...
    def mostrar_tela_resultados_em_progresso(self):
        ...
        Button(
            frame_topo,
            text="📋 Gerar Relatório (.docx)",
            command=self.gerar_documento_word,
            font=tkfont.Font(size=12, weight="bold"),
            bg="#2E7D32",
            fg="white",
        ).pack(side="left")
        self.status_label = Label(
            frame_topo, text="Iniciando...", font=tkfont.Font(size=11)
        )
        ...
        headings = {
            ...
            "nome": ("Nome do Condutor", 280),
            "remessa": ("Remessa SIT Próxima", 150),
            "status": ("Status", 100),
            "dias": ("Dias", 120),
        }
        ...
        scrollbar = ttk.Scrollbar(
            frame_tabela, orient="vertical", command=self.treeview_resultados.yview
        )
    ...
    def iniciar_consulta_processo(self):
        ...
        total_inicial = len(self.dados_processos.get("result", [])) + len(
            self.resultados_finais
        )
        self.status_label.config(
            text=f"Processando {len(self.resultados_finais) + 1} de {total_inicial}..."
        )
    ...
    def _worker_consultar_processo(self):
        try:
            ...
            wait.until(
                EC.element_to_be_clickable(
                    (By.ID, "ctl00_conteudo_ucPesquisa_txbNumProtocolo")
                )
            ).send_keys(protocolo)
            wait.until(
                EC.element_to_be_clickable(
                    (By.ID, "ctl00_conteudo_ucPesquisa_btnFiltrar")
                )
            ).click()
            nome_condutor = wait.until(
                EC.presence_of_element_located(
                    (By.ID, "ctl00_conteudo_ucDados_lblNomeTexto")
                )
            ).text.strip()
            infracoes = self._extrair_infracoes(wait)
            dados_analisados = self._analisar_datas_infracoes(
                protocolo, nome_condutor, infracoes
            )
    ...
    def verificar_resultado_consulta(self):
        try:
            ...
            if resultado["status"] == "sucesso_consulta":
                ...
                messagebox.showerror(
                    "Falha na Consulta", resultado.get("erro", "Erro desconhecido.")
                )
                self.mostrar_tela_login()
        except queue.Empty:
    ...
    def gerar_documento_word(self):
        if not self.resultados_finais:
            messagebox.showwarning(
                "Atenção", "Nenhum dado foi processado para gerar o relatório."
            )
            return
        try:
            filepath = filedialog.asksaveasfilename(
                ...
                initialfile="Relatorio_Processos.docx",
            )
            if not filepath:
                return
            ...
            doc.save(filepath)
            messagebox.showinfo(
                "Sucesso", f"Relatório salvo com sucesso em:\n{filepath}"
            )
        except Exception as e:
            messagebox.showerror(
                "Erro ao Gerar Documento",
                f"Não foi possível criar o arquivo Word.\n\nErro: {e}",
            )
